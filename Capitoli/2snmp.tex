\section{Gestione della rete e protocollo SNMP}

\subsection{Introduzione alla gestione della rete (Network Management)}

\subsubsection{Definizione e motivazioni}

Una rete di calcolatori è un sistema intrinsecamente complesso, caratterizzato
da migliaia di componenti hardware e software che interagiscono attraverso una
struttura a livelli. Questa complessità, che aumenta esponenzialmente con il
numero di interconnessioni tra i vari livelli e sistemi, rende la gestione
...
La disciplina che si occupa di questa sfida è il \textbf{Network Management (NM)},
formalmente definito come l'insieme di attività di deployment, integrazione e
coordinamento degli elementi hardware, software e umani per monitorare, testare,
interrogare, configurare, analizzare, valutare e controllare la rete e le sue
...
ma anche il consumo energetico e delle risorse.

La complessità della gestione di una rete moderna è paragonabile a quella di
altri sistemi critici su larga scala, come un aereo di linea o una centrale
nucleare. Così come questi sistemi richiedono un monitoraggio e un controllo
costanti per operare in sicurezza ed efficienza, anche una rete necessita di una
supervisione continua per prevenire e risolvere problemi.
...
\subsubsection{Le cinque aree funzionali}

Per organizzare in modo strutturato le complesse attività di network management,
l'ISO ha sviluppato un modello di riferimento che suddivide il dominio in cinque
aree funzionali. Queste aree, note con l'acronimo \textbf{FCAPS}, rappresentano
le principali aree di interesse della gestione di rete e forniscono un framework
concettuale per classificare compiti, strumenti e policy.

\paragraph{Fault Management (gestione dei guasti)}

L'obiettivo di quest'area è rilevare, registrare, isolare e rispondere alle
condizioni di guasto nella rete. Sebbene la linea di demarcazione con il
Performance Management sia a volte sottile, la gestione dei guasti si concentra
sulla gestione mirata dei fallimenti transitori. È importante sottolineare che
...
incluse le loro configurazioni hardware e software. Mantenere un inventario
aggiornato e garantire la coerenza delle configurazioni è cruciale per la
stabilità della rete. In questo contesto, l'AI può fornire un supporto
significativo, ad esempio aiutando a prendere decisioni complesse come la scelta
ottimale di un server di backup in base a carichi e disponibilità.
...
intraprendono azioni correttive (esempio: modificando una configurazione).
...
\subsection{Standard di gestione e architettura SNMP}

\subsubsection{Contesto storico e componenti chiave}

Per gestire un sistema efficace in un ambiente eterogeneo, è indispensabile
l'adozione di protocolli standard. Negli anni '80, l'approccio OSI con il suo
CMIP (\textit{Common Management Information Protocol}) mirava a diventare lo
standard unificato, ma il suo processo di standardizzazione fu troppo lento per
le esigenze di mercato. In parallelo, l'approccio Internet diede vita a
\textbf{SNMP} (\textit{Simple Network Management Protocol}), che partendo da
una base più semplice ottenne una rapida adozione e divenne lo standard de facto.

SNMP deriva dal precedente SGMP (\textit{Simple Gateway Monitoring Protocol}).
La sua semplicità iniziale ne favorì la diffusione, ma con il tempo crebbe in
dimensioni e complessità per rispondere a nuove esigenze. La versione attualmente
più diffusa e completa è \textbf{SNMPv3}, che ha introdotto fondamentali
...
Questi dati includono contatori (esempio: pacchetti inviati/ricevuti),
informazioni di stato, versioni software e molto altro.
...
\subsection{Analisi dei componenti fondamentali di SNMP}

\subsubsection{La struttura dei dati: SMI e MIB}
...
lo SMI (\textit{Structure of Management Information}) e la MIB
(\textit{Management Information Base}).

Lo SMI è un linguaggio di definizione dei dati il cui scopo è specificare la
sintassi e la semantica delle informazioni di gestione. Invece di reinventare la
ruota, SMI definisce un sottoinsieme di tipi di dati standard
(esempio: INTEGER, OCTET STRING, Counter32) e due costrutti principali per
...
Data la vastità di dispositivi e protocolli esistenti, sorge un problema
fondamentale: come assegnare un nome univoco a ogni possibile oggetto di
gestione standardizzato, in modo che sia valido in qualsiasi parte del mondo?

La soluzione adottata è l'\textbf{ISO Object Identifier (OID) Tree}, una
...
\item \textbf{OID}: \texttt{.1.3.6.1.2.1.1.3 ::= \{ system 3 \}}, che lo
posiziona come terzo oggetto nel gruppo system.
\end{itemize}

Il gruppo \texttt{system} nel suo insieme fornisce informazioni di base
essenziali su un dispositivo gestito, agendo come una sorta di ``targhetta
digitale''. La prima interrogazione che un gestore di rete esegue su un nuovo
dispositivo è tipicamente rivolta a questo gruppo. L'oggetto \textbf{sysDescr}
fornisce una descrizione completa dell'hardware, del sistema operativo e del
software di rete, permettendo di identificare immediatamente il tipo di
apparato. \textbf{sysObjectID} definisce un identificatore univoco assegnato
dal produttore, cruciale per l'identificazione automatizzata tramite script.
\textbf{sysContact} e \textbf{sysLocation} forniscono il contesto
amministrativo. Infine, \textbf{sysServices} riassume le funzionalità offerte
dal nodo, offrendo una visione d'insieme delle sue capacità.
...
(come HMAC con SHA o MD5) applicata al messaggio insieme a una chiave segreta
condivisa.
...
permette di definire policy di accesso granulari. Un'entità SNMP può
mantenere un database che specifica i diritti di accesso per utenti diversi,
limitando non solo quali operazioni possono eseguire (lettura, scrittura) ma
anche quali porzioni dell'albero MIB (viste) possono accedere.
\end{itemize}
...
(\textit{Abstract Syntax Notation One}), uno standard ISO (X.680) che permette
...
Le BER utilizzano una codifica \textbf{TLV} (\textit{Type, Length, Value}), un
modo standard per formattare i dati in modo auto-identificante.
Ogni dato trasmesso è composto da tre campi:
\begin{itemize}
...
vengono scambiati su un canale di trasporto sicuro e affidabile (come SSH, TLS,
BEEP/TLS o SOAP/HTTP/TLS) per eseguire operazioni sulla configurazione del
