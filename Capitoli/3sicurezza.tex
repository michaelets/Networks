...
\paragraph{Riservatezza (Confidentiality)}
...
\paragraph{Disponibilità (Availability)}
...
Questi attacchi sono quasi impossibili da rilevare; l'unica difesa efficace è
la prevenzione, tipicamente attraverso la cifratura.
...
\item \textbf{Non-Repudiation}: impedisce la negazione di transazioni effettuate.
\end{itemize}
...
\item \textbf{Controllo del routing}: selezione di percorsi sicuri e separati.
...
\begin{itemize}
\item \textbf{Integrità}: include la modifica dei dati utente o della memoria
e l'uso di Trojan horse nel browser. Le conseguenze comprendono la perdita di
informazioni, la compromissione della macchina e l'amplificazione delle
vulnerabilità. La contromisura principale è l'uso di checksum crittografici.
\item \textbf{Riservatezza}: minacce come eavesdropping, furto di dati dal
server o dal client e analisi della configurazione portano alla perdita di
informazioni sensibili e al decadimento della privacy. Le contromisure includono
la crittografia e i web proxy.
\item \textbf{Denial of Service}: flooding, attacchi DNS e interruzione di
thread utente causano interruzione dell'attività e degradazione delle prestazioni.
La rilevazione tramite Intrusion Detection è l'approccio principale, data la
difficoltà di prevenzione assoluta.
\item \textbf{Autenticazione}: l'impersonificazione di utenti legittimi e la
falsificazione di dati portano all'appropriazione di diritti e alla validazione
di dati falsi. La contromisura è l'uso di tecniche crittografiche di validazione.
\end{itemize}
