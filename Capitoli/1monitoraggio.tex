...
\item Dati relativi al personale (esempio: ruoli, responsabilità).
\item Calendari interni (esempio: di manutenzione).
\end{itemize}
...
\section{La rete di sensori (Sensor Network)}
...
\subsection{Classificazione dei sensori: un modello a tre attributi}
...
\subsection{Analisi del dominio (Domain)}
...
\subsection{Analisi per posizione (Vantage)}
...
\subsubsection{Vantage a livello Data Link}
...
\subsubsection{Vantage a livello IP}
...
\subsection{Analisi per azione (Action)}
...
\section{Formati dei dati e protocolli di monitoraggio}
...
\subsubsection{Dettagli su NetFlow v5}
...
\subsubsection{Evoluzione di NetFlow}
...
\subsubsection{Problematiche di generazione NetFlow}
...
\subsection{Confronto e limiti dei metodi tradizionali}
...
\section{Tecniche di monitoraggio evolute: In-Band Network Telemetry (INT)}
...
\subsection{Modalità operative di INT}
...
\subsection{Sfide e svantaggi di INT}
...
\section{Tecnologie abilitanti per il data plane programmabile}
