...
\subsubsection{Identifying Descriptors in Time Series}

L'immagine associata a questa sezione mostra un grafico tipico
...
\subsubsection{Suitable Only When Observable Trend Arise}
...
normale standard, i punti che risiedono fuori dall'intervallo
...
distribuzione $F(x, d)$ sono tabulati e disponibili in
letteratura. Per applicare il test, è necessario definire un
...
un sottoinsieme $X_i$ con $\psi$ campioni, effettuato senza
reimmissione (without replacement). La dimensione raccomandata
per $\psi$ varia tra 128 e 512 elementi. L'altezza massima
dell'albero viene quindi impostata a $L = \log_2 \psi$.
...
ottenendo $E[h(x)]$. Il punteggio di anomalia $s(x, \psi)$
è definito come:
\[
...
$c(\psi)$, allora $s$ tende a 1, indicando che $x$ è
...
K-distanza di $B$. Questa formulazione serve ad aumentare la
...
la LRD del punto $A$ stesso. L'interpretazione del rapporto è:
...
dai cluster dei dati. Tuttavia, questo metodo è generale ma
presenta positivi. Sebbene rilevi sia anomalie locali che
globali, solitamente sa bene per le prime più che per le
seconde. Inoltre, il metodo ha difficoltà a rilevare i bordi
...
modello AE è che i livelli nascosti abbiano meno dimensioni
...
normale poiché ne abbiamo solitamente in abbondanza.
...
\subsubsection{Evaluation Graphs}
